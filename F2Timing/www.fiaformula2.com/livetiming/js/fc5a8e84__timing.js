...
$(function () {
    ...
    lt.client.sessionfeed = function (sentTime, value) {
        try {
            ...
            if (timingrootScope) timingrootScope.$apply();
        }
        catch (ex) { console.error('outer', ex.message); }
    };
    ...
    lt.client.datafeed = function (sentTime, lineNo, feed) {
        ...
        try {
            ...
            if (feed && feed.Session) {
                ...
                } else if (feed.Session == 'F1Qualifing') {
                    timingScope.feed.Session = 'F1Qualifing';
                } else {
    ...
    timingApp.controller('racedetails', function ($scope) {
        $scope.racedetails = racedetails;
    });
...
* File mirrored from https://www.fiaformula2.com/livetiming/js/timing.js
* At UTC time: 2021-07-10 11:41:50.433015
*/
